#pragma once
#include "Engine/Core/Entity-Component-System/headers/PagedAllocator.hpp"
...
namespace gcep
{
    ...
    template<typename T>
    bool PagedAllocator<T>::hasPage(size_t pageNumber) const
    {
        if (pageNumber >= m_pages.size()) {
            return false;
        }

        return m_pages[pageNumber] != nullptr;
    }
    ...
    T& PagedAllocator<T>::get(size_t index)
    {
        ...
        allocatePages(pageNumber); // OK ici
    ...
    void PagedAllocator<T>::reserve(size_t entityCount) {
        // Combien de pages nous faut-il pour stocker autant d'entités ?
        size_t lastPageNeeded = entityCount >> PAGE_SHIFT;

        // On agrandit le vecteur de pointeurs de pages si nécessaire
        if (lastPageNeeded >= m_pages.size()) {
            m_pages.resize(lastPageNeeded + 1, nullptr);
        }

        // On alloue chaque page qui n'existe pas encore
        for (size_t i = 0; i <= lastPageNeeded; ++i) {
            if (m_pages[i] == nullptr) {
                m_pages[i] = new T[PAGE_SIZE];
                // On initialise avec la valeur par défaut
