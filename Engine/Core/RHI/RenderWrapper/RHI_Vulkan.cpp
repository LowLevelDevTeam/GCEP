...
void RHI_Vulkan::updateUniformBuffer(uint32_t currentImage)
{
    ...
    // Use offscreen extent for projection since we render to offscreen image
...
void RHI_Vulkan::createDescriptorSets()
{
    ...
    m_descriptorSets = std::make_unique<vk::raii::DescriptorSets>(m_device.allocateDescriptorSets(allocInfo));
...
void RHI_Vulkan::recordOffscreenCommandBuffer()
{
    ...
    if (!m_descriptorSets || m_descriptorSets->empty())
    {
        std::cerr << "ERROR: m_descriptorSets is null or empty!" << std::endl;
        return;
    }
    ...
    // Transition offscreen imageimage to color attachment
